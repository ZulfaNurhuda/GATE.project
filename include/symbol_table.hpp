...
struct SymbolInfo {
    ...
    std::vector<SymbolInfo> parameters; // For functions/procedures
...
public:
    ...
    SymbolInfo* lookupSymbol(const std::string& name) const;

    // Looks up a symbol only in the current (innermost) scope.
    // Returns a pointer to SymbolInfo if found, otherwise nullptr.
    const SymbolInfo* lookupSymbolInCurrentScope(const std::string& name) const;
